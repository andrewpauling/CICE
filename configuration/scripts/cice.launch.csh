...
cat >> ${jobfile} << EOFR
mpirun -np ${ntasks} --bind-to none --map-by slot apptainer exec --bind \${I_MPI_ROOT}:\${I_MPI_ROOT} --bind /mnt/data_01 \${SIF_FILE} ${ICE_RUNDIR}/cice  >&! \$ICE_RUNLOG_FILE
EOFR
endif

else if (${ICE_MACHCOMP} =~ icebox*) then
if (${ICE_COMMDIR} =~ serial*) then
cat >> ${jobfile} << EOFR
./cice >&! \$ICE_RUNLOG_FILE
EOFR
